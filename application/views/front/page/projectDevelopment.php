...
$plnSess = $this->session->userdata('pDplnSess');
?>
<div class="col-md-9 col-sm-6 col-xs-12">
    <div class="form-section ">
        <?php echo form_open_multipart(base_url().'searchProjectDevelopment',array('method'=>'post', 'name'=>'searchPlanning','id'=>'searchPlanning','class'=>'','autocomplete'=>'off'));?>
        <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-7">
                        <div class="form-group">
                            <input type="text" name="pdid" value="<?php echo $this->session->userdata('pDpdid');?>" class="form-control search"/>
                        </div>
                    </div>
                </div>
                <div class="row">
                        ...
                        <div class="form-group">
                            <select name="client_id" class="form-control search">
                                ...
                                <?php foreach($clients as $client) {?>
                                    <option value="<?php echo $client['user_id'];?>" <?php if($this->session->userdata('pDclient_id')==$client['user_id']){ echo "selected=selected";}?>><?php echo ucwords($client['fname'].' '.$client['lname']);?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            <select name="brand_id" class="form-control search">
                                ...
                                <?php foreach($brands as $brand) {?>
                                    <option value="<?php echo $brand['brand_id'];?>" <?php if($this->session->userdata('pDbrand_id')==$brand['brand_id']){ echo "selected=selected";}?>><?php echo $brand['title'];?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            <select name="zone_id" class="form-control search">
                                ...
                                <?php foreach($zones as $zone) {?>
                                    <option value="<?php echo $zone['zone_id'];?>" <?php if($this->session->userdata('pDzone_id')==$zone['zone_id']){ echo "selected=selected";}?>><?php echo $zone['title'];?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            <select name="state_id" class="form-control search">
                                ...
                                <?php foreach($states as $state) {?>
                                    <option value="<?php echo $state['state_id'];?>" <?php if($this->session->userdata('pDstate_id')==$state['state_id']){ echo "selected=selected";}?>><?php echo $state['title'];?></option>
                                <?php }?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-7">
                        <div class="form-group">
                            <select name="status" class="form-control search">
                                <option value="">-Select-</option>
                                <option value="Pending" <?php if($this->session->userdata('pDstatus')=='Pending'){ echo "selected=selected";}?>>Pending</option>
                                <option value="WIP" <?php if($this->session->userdata('pDstatus')=='WIP'){ echo "selected=selected";}?>>WIP</option>
                                <option value="Completed" <?php if($this->session->userdata('pDstatus')=='Completed'){ echo "selected=selected";}?>>Completed</option>
                            </select>
                        </div>
                        <div class="form-group  pull-right">
                        <button type="submit" class="btn btn-warning btn-sm">Search</button>
                            <?php if(!empty($plnSess)){ ?><button type="button" class="btn btn-dark-gray btn-sm" onclick="window.location='<?php echo base_url();?>unsetProjectDevelopment'">Reset</button><?php }?>
                        ...
                        <table class="table table-responsive table-bordered">
                            ...
                            <?php 
                                ...
                                <tr>
                                    ...
                                    <td><a href="<?php echo base_url();?>project-development/<?php echo $planning['brief_module_id'];?>" class="btn btn-orange btn-xs">Open</a></td>
